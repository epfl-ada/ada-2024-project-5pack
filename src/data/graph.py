...
def _get_edge_weights(graph_data: dict, paths_finished: bool) -> tuple[dict[edge, int], set[edge]]:
	"""
	...
	assert(sum(edge_weights.values()) == 
        sum(graph_data[dataframe_key]["path"].apply(len)) 
        - sum(graph_data[dataframe_key]["path"].apply(lambda list: list.count("<"))) 
