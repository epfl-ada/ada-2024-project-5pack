...
from src.models.semantic_analyzer import SemanticAnalyzer
...
def analyze_network(graph_data, results_dir):
	...
	logger.info(
		f"Using pre-built graph with {network_analyzer.graph.number_of_nodes()} nodes and "
...
def analyze_paths(graph_data, network_analyzer, results_dir):
	...
	path_analyzer = PathAnalyzer(semantic_analyzer=semantic_analyzer, network_analyzer=network_analyzer)
	...
	paths_df = path_analyzer.analyze_paths(
		paths_finished=graph_data["paths_finished"], paths_unfinished=graph_data["paths_unfinished"]
	)
...
def perform_statistical_analysis(paths_df, results_dir):
	...
	semantic_results = stat_analyzer.test_semantic_proximity_hypothesis(
		paths_df,
		paths_df["semantic_distance"].to_dict(),  # Using computed semantic distances
	)
	...
	results = {
		...
		"semantic_analysis": {
			...
			"p_value": float(semantic_results["p_value"]),
			"effect_size": float(semantic_results["effect_size"]),
			"sample_size": int(semantic_results["sample_size"]),
		},
		"model_performance": {
			"roc_auc": float(model_results["roc_auc"]),
			"cv_scores": {"mean": float(model_results["cv_scores"]["mean"]), "std": float(model_results["cv_scores"]["std"])},
	...
	return strategy_results, semantic_results, model_results


def create_visualizations(paths_df, network_analyzer, model_results, path_stats, time_patterns, results_dir):
	...
	try:
		...
		nav_stats = {"path_stats": path_stats, "time_patterns": time_patterns}
		figs["navigation_patterns"] = visualizer.plot_navigation_patterns(paths_df, nav_stats)

		# Network structure
		figs["network_structure"] = visualizer.plot_network_structure(network_analyzer.graph, network_analyzer.centrality_metrics)
...
def generate_report(network_metrics, path_stats, strategy_results, model_results, time_patterns, results_dir):
	"""Generate analysis report."""
	logger.info("Generating analysis report...")

	# Convert feature importance list to a more suitable format if it exists
	feature_importance = model_results.get("feature_importance", [])
	if isinstance(feature_importance, list):
		feature_importance_dict = {f"feature_{i}": feat for i, feat in enumerate(feature_importance)}
...
def main():
	...
	try:
		...
		paths_df, path_stats, time_patterns = analyze_paths(graph_data, network_analyzer, results_dir)
		strategy_results, model_results = perform_statistical_analysis(paths_df, results_dir)

		# Create visualizations
		create_visualizations(paths_df, network_analyzer, model_results, path_stats, time_patterns, results_dir)

		# Generate final report
		generate_report(
			network_analyzer.compute_network_metrics(), path_stats, strategy_results, model_results, time_patterns, results_dir
		)
