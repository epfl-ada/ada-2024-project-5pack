...
class StatisticalAnalyzer:
	...
	def test_semantic_proximity_hypothesis(self, paths_df, semantic_distances):
		...
		try:
			...
			paths_df["semantic_distance"] = paths_df.apply(
			...
			pooled_std = np.sqrt(
				(successful.var() * (len(successful) - 1) + failed.var() * (len(failed) - 1))
			...
			logger.error(f"Error in semantic proximity test: {e!s}")
			raise ValueError(f"Semantic proximity analysis failed: {e!s}")
	...
	def fit_logistic_regression(self, paths_df):
		...
		try:
			...
			logger.error(f"Error in logistic regression analysis: {e!s}")
			raise ValueError(f"Logistic regression failed: {e!s}")

	def test_strategy_effectiveness(self, paths_df: pd.DataFrame):
		...
		try:
			...
			success_rates = paths_df.groupby("strategy")["success"].agg(
				["mean", "count"],
			)
			...
			logger.info(
				f"Strategy effectiveness test completed: p-value = {p_value:.4f}",
			)
			return results

		except Exception as e:
			logger.error(f"Error in strategy effectiveness test: {e!s}")
			raise ValueError(f"Strategy analysis failed: {e!s}")
	...
	def predict_success_probability(self, features):
		...
		if self.model is None:
			raise ValueError("Model not fitted. Call `fit_logistic_regression` first.")
		...
		try:
			...
			logger.error(f"Error in prediction: {e!s}")
			raise ValueError(f"Prediction failed: {e!s}")
