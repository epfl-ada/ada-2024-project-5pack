...
def get_hub_articles(top_n=50):
    ...
    if top_n == HUB_COUNTS_TO_TEST[0]:
        ...
        plt.title('PageRank Score Distribution of Articles\n(Top 200 Articles)', fontsize=14, pad=20)
        plt.xlabel('Article Rank (Most Central to Least Central)', fontsize=12)
        plt.ylabel('PageRank Score', fontsize=12)
        plt.axhline(y=scores[top_n], color='#e74c3c', linestyle='--', 
...
def find_optimal_parameters():
    ...
    paths_finished = graph_data['paths_finished'].copy()
    paths_finished['success'] = True
    
    paths_unfinished = graph_data['paths_unfinished'].copy()
    ...
    # Test combinations
    results = []
    for n_hubs in HUB_COUNTS_TO_TEST:
        for threshold in THRESHOLDS_TO_TEST:
            stats = analyze_hub_strategy(paths_df, n_hubs, threshold)
    ...
    results_df['score'] = (results_df['success_rate'] * 0.7 + 
                          results_df['paths_percentage'] * 0.3)
...
if __name__ == "__main__":
    ...
    print(f"Percentage of paths using strategy: {optimal_params['paths_percentage']:.2%}")
    
    # Save detailed results
    results_df.to_csv('hub_strategy_analysis.csv', index=False)