...
class StatisticalAnalyzer:
	...
	def test_semantic_proximity_hypothesis(self, paths_df, semantic_distances):
		...
		semantic_distances : dict
		    Dictionary of semantic distances between articles
		...
		try:
			...
			def get_semantic_distance(row):
				...
				if len(path_articles) < 2:
					return None
				start = path_articles[0]
				end = path_articles[-1]
			...
			# Compare distances for successful vs failed paths
	...
	def fit_logistic_regression(self, paths_df):
		...
		try:
			...
			X_train, X_test, y_train, y_test = train_test_split(
				X, y, test_size=ANALYSIS_PARAMS["test_size"], random_state=ANALYSIS_PARAMS["random_state"]
			)
			...
			for name, coef in zip(X.columns, self.model.coef_[0]):
				feature_importance.append({"feature": name, "coefficient": float(coef), "abs_importance": float(abs(coef))})

			# Sort by absolute importance
			feature_importance.sort(key=lambda x: x["abs_importance"], reverse=True)

			# Cross validation
			cv_scores = cross_val_score(self.model, X_train_scaled, y_train, cv=ANALYSIS_PARAMS["cv_folds"])

			results = {
				"feature_importance": feature_importance,
				"classification_report": classification_report(y_test, y_pred),
				"roc_auc": roc_auc_score(y_test, y_pred_proba),
				"cv_scores": {"mean": float(cv_scores.mean()), "std": float(cv_scores.std()), "scores": cv_scores.tolist()},
			}
