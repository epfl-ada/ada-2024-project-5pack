...
STRATEGY_THRESHOLDS = {
	...
	"exploratory_backtrack_ratio": 0.3,
	"hub_usage_threshold": 0.5,
}
...
class PathAnalyzer:
	...
	def compute_basic_metrics(self, paths_df):
		...
		try:
			...
			paths_df["backtrack_count"] = paths_df["path"].apply(
			...
			if self.semantic_analyzer is not None:
				paths_df["semantic_coherence"] = paths_df["path"].apply(
					self.semantic_analyzer.get_path_similarity,
				)
			else:
				logger.warning(
					"No semantic analyzer available, semantic_coherence will not be computed",
				)
		...
		try:
			...
			def get_strategy(row):
				...
				if row["backtrack_count"] == 0 and row["path_efficiency"] > direct_thresh:
					return "Direct"
				if backtrack_ratio > explore_thresh:
					return "Exploratory"
				if "hub_usage_ratio" in row and row["hub_usage_ratio"] > hub_thresh:
					return "Hub-focused"
				return "Mixed"
			...
			stats = {
				"basic_stats": {
					...
					"avg_backtrack_rate": float(
						paths_df["backtrack_count"].sum() / paths_df["path_length"].sum(),
					),
				},
	...
	def analyze_time_patterns(self, paths_df):
		...
		try:
			...
			logger.error(f"Error analyzing time patterns: {e!s}")
			raise ValueError(f"Time pattern analysis failed: {e!s}")
