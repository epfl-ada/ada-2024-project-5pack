...
class WikispeediaVisualizer:
	...
	def plot_navigation_patterns(self, paths_df, stats):
		...
		try:
			...
			ax4.pie(strategy_counts, labels=strategy_counts.index, autopct="%1.1f%%", colors=self.colors)
	...
	def plot_model_performance(self, model_results):
		...
		try:
			...
			if "roc_auc" in model_results:
				ax3.text(0.5, 0.5, f"ROC AUC: {model_results['roc_auc']:.3f}", horizontalalignment="center", fontsize=14)
				ax3.axis("off")
